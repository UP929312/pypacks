...
def generate_resource_pack(pack: "Pack") -> None:
    ...
    # ================================================================================================
    # pack.mcmeta
    with open(pack.resource_pack_path/"pack.mcmeta", "w", encoding="utf-8") as file:
    ...
    if pack.custom_sounds:
        with open(pack.resource_pack_path/"assets"/pack.namespace/"sounds.json", "w", encoding="utf-8") as file:
...
def generate_base_font(pack: "Pack") -> "CustomFont":
    ...
    all_elements = [
        SpaceFontChar(),
        AutoAssignBitMapFontChar("blank_icon", Path(IMAGES_PATH, "reference_book_icons", "blank_icon.png").read_bytes()),
        AutoAssignBitMapFontChar("logo_256_x_256", Path(IMAGES_PATH, "reference_book_icons", "logo_256_x_256.png").read_bytes(), height=100, y_offset=16),
        AutoAssignBitMapFontChar("satchel_icon", add_border(image_bytes=Path(IMAGES_PATH, "reference_book_icons", "satchel.png").read_bytes()), height=20, y_offset=10),
        AutoAssignBitMapFontChar("information_icon", add_border(image_bytes=Path(IMAGES_PATH, "reference_book_icons", "information_icon.png").read_bytes(),
                                                                base_image_path=EXTRA_ICON_BASE_PATH), height=18, y_offset=14),
        (
            AutoAssignBitMapFontChar("dimensions_icon", add_border(image_bytes=Path(IMAGES_PATH, "reference_book_icons", "dimensions_icon.png").read_bytes()),
                                     height=20, y_offset=10)
            if pack.custom_dimensions else None
        ),
        (
            AutoAssignBitMapFontChar("play_icon", add_border(image_bytes=Path(IMAGES_PATH, "reference_book_icons", "play_icon.png").read_bytes(),
                                                             base_image_path=EXTRA_ICON_BASE_PATH), height=18, y_offset=14)
        ...
        *[  # Category icons (Items and Dimensions)
            AutoAssignBitMapFontChar(f"{category.internal_name}_category_icon", image_bytes=add_border(Path(category.image_path).read_bytes()), height=20, y_offset=10)
        ...
        *[  # Custom recipes
            AutoAssignBitMapFontChar(f"custom_recipe_for_{custom_recipe.internal_name}_icon", image_bytes=custom_recipe.generate_recipe_image(), y_offset=6)
...
def generate_datapack(pack: "Pack") -> None:
    ...
    # ================================================================================================
    # pack.mcmeta
    with open(pack.datapack_output_path/"pack.mcmeta", "w", encoding="utf-8") as file:
