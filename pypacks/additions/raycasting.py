...
class Raycast:
    ...
    def generate_default_raycast_functions(pack_namespace: str) -> tuple["MCFunction", ...]:
        ...
        ray = MCFunction("ray", [
            ...
            f"$execute $(success_condition) run execute as @s run function {on_success_set_score.get_reference(pack_namespace)} {{\"on_successful_hit\": \"$(on_successful_hit)\"}}",
            "",
            ...
            "$execute if score #hit raycast matches 0 if score #distance raycast matches $(max_iterations_plus_one).. run execute as @s run $(on_failure)",
            "",
            "# Advance forward and run the ray again if no successful hit occured",
            f"$execute if score #hit raycast matches 0 if score #distance raycast matches ..$(max_iterations) positioned ^ ^ ^$(distance_between_iterations) run execute as @s run function {pack_namespace}:raycast/ray {formatted_arguments_replacements}",
            ],
            ["raycast"],
        )
